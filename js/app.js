...
// === АКТИВИ ===
function parseAsset(page) {
  const p = page.properties;
  return {
    id: page.id,
    symbol: getTitle(p, COL.AS_SYMBOL),
    name: getRichText(p, COL.AS_NAME),
    cgId: getRichText(p, COL.AS_CG_ID),
    type: getSelect(p, COL.AS_TYPE) || 'crypto',
    icon: getRichText(p, COL.AS_ICON),
    tgId: getNumber(p, COL.TG_ID),
    // обчислювальні
    transactions: [],
    qty: 0,
...
function buildAssetProps(a) {
  return {
    [COL.AS_SYMBOL]: pTitle(a.symbol),
    [COL.AS_NAME]: pRichText(a.name || ''),
    [COL.AS_CG_ID]: pRichText(a.cgId || ''),
    [COL.AS_TYPE]: pSelect(a.type || 'crypto'),
    [COL.AS_ICON]: pRichText(a.icon || ''),
    [COL.TG_ID]: pNumber(currentTgId)
  };
}
function parseAssetTx(page) {
  const p = page.properties;
  return {
    id: page.id,
    assetId: getRelationFirst(p, COL.AT_ASSET),
    date: getDate(p, COL.AT_DATE),
    txType: getSelect(p, COL.AT_TXTYPE) || 'buy',
    qty: getNumber(p, COL.AT_QTY) || 0,
    price: getNumber(p, COL.AT_PRICE) || 0,
    cur: getSelect(p, COL.AT_CUR) || 'USD',
    fee: getNumber(p, COL.AT_FEE) || 0,
    note: getRichText(p, COL.AT_NOTE),
    tgId: getNumber(p, COL.TG_ID)
  };
}
function buildAssetTxProps(assetId, tx) {
  return {
    [COL.AT_ID]: pTitle('at-' + Date.now().toString(36)),
    [COL.AT_ASSET]: pRelation(assetId),
    [COL.AT_DATE]: pDate(tx.date),
    [COL.AT_TXTYPE]: pSelect(tx.txType),
    [COL.AT_QTY]: pNumber(tx.qty),
    [COL.AT_PRICE]: pNumber(tx.price),
    [COL.AT_CUR]: pSelect(tx.cur),
    [COL.AT_FEE]: pNumber(tx.fee || 0),
    [COL.AT_NOTE]: pRichText(tx.note || ''),
    [COL.TG_ID]: pNumber(currentTgId)
  };
}
...
async function loadAllFromNotion() {
  try {
    ...
    // Активи — окремо, тільки якщо увімкнено
    if (state.settings && state.settings.assetsEnabled) {
      loadAssetsFromNotion().catch(e => console.warn('assets load failed', e));
    }
    return true;
  } catch (e) {
...
async function loadAssetsFromNotion() {
  try {
    const [aRaw, atRaw] = await Promise.all([
      notionQueryAll('DB_ASSETS'),
      notionQueryAll('DB_ASSET_TX')
    ]);
    const mine = x => x.tgId === currentTgId;
    const assets = aRaw.map(parseAsset).filter(mine);
    const txs = atRaw.map(parseAssetTx).filter(mine);
    ...
    state.assets = assets;
    state.assetTransactions = txs;

    // Тягнемо поточні ціни
    await refreshAssetPrices();

    if (typeof renderAssets === 'function') renderAssets();
  } catch (e) {
    console.warn('Активи не доступні:', e);
    state.assets = [];
    state.assetTransactions = [];
  }
}

// Перерахунок статистики активу з його транзакцій
function recalcAssetStats(a) {
  let qty = 0;
  let investedUSD = 0;
  let totalBuyCostUSD = 0;
  let totalBuyQty = 0;
  for (const t of a.transactions) {
    const priceUSD = convert(t.price, t.cur || 'USD', 'USD');
    const feeUSD = convert(t.fee || 0, t.cur || 'USD', 'USD');
    if (t.txType === 'buy') {
      qty += t.qty;
      investedUSD += t.qty * priceUSD + feeUSD;
      totalBuyCostUSD += t.qty * priceUSD;
      totalBuyQty += t.qty;
    } else if (t.txType === 'sell') {
      qty -= t.qty;
      investedUSD -= (totalBuyQty > 0 ? (totalBuyCostUSD / totalBuyQty) : 0) * t.qty;
    }
  }
  a.qty = Math.max(0, qty);
  a.avgBuyUSD = totalBuyQty > 0 ? totalBuyCostUSD / totalBuyQty : 0;
  a.investedUSD = Math.max(0, investedUSD);
  // ціна і valueUSD оновлюються окремо в refreshAssetPrices
  const price = state.assetPrices[a.cgId]?.price || a.currentPriceUSD || 0;
  a.currentPriceUSD = price;
  a.valueUSD = a.qty * price;
  a.pnlUSD = a.valueUSD - a.investedUSD;
  a.pnlPct = a.investedUSD > 0 ? (a.pnlUSD / a.investedUSD) * 100 : 0;
}

// Оновити ціни всіх активів (не частіше ніж раз на хвилину)
async function refreshAssetPrices() {
  const cgIds = state.assets.map(a => a.cgId).filter(Boolean);
  if (cgIds.length === 0) return;
  try {
    const prices = await cgPrices(cgIds);
    for (const id in prices) {
      state.assetPrices[id] = { ...prices[id], ts: Date.now() };
    }
...
// === АКТИВИ CRUD ===
async function createAssetInNotion(a) {
  try {
    const res = await notionCall({ action: 'create', database: 'DB_ASSETS', properties: buildAssetProps(a) });
    if (res && res.id) a.id = res.id;
  } catch (e) { toast('Notion: не вдалось зберегти актив', 'error'); console.warn(e); }
}
async function updateAssetInNotion(a) {
  if (!a.id || a.id.length < 30) return;
  try {
    await notionCall({ action: 'update', pageId: a.id, properties: buildAssetProps(a) });
  } catch (e) { console.warn(e); }
}
async function deleteAssetInNotion(assetId) {
  if (!assetId || assetId.length < 30) return;
  try {
    await notionCall({ action: 'archive', pageId: assetId });
  } catch (e) { console.warn(e); }
}
async function createAssetTxInNotion(assetId, tx) {
  if (!isNotionId(assetId)) {
    for (let i = 0; i < 10; i++) {
      await new Promise(r => setTimeout(r, 500));
      const a = state.assets.find(x => x.id === assetId);
      if (a && isNotionId(a.id)) { assetId = a.id; break; }
    }
    if (!isNotionId(assetId)) {
      showErrorBanner('Актив ще створюється. Зачекай і повтори.');
      return;
    }
  }
  try {
    const res = await notionCall({ action: 'create', database: 'DB_ASSET_TX', properties: buildAssetTxProps(assetId, tx) });
    if (res && res.id) tx.id = res.id;
  } catch (e) { console.warn(e); }
}
async function deleteAssetTxInNotion(txId) {
  if (!txId || txId.length < 30) return;
  try {
    await notionCall({ action: 'archive', pageId: txId });
  } catch (e) { console.warn(e); }
...
// === COINGECKO API (через проксі) ===
async function cgCall(path, query) {
...
// Пошук монет ("bitc" → [{ id, symbol, name, thumb }])
async function cgSearch(q) {
  if (!q || q.length < 2) return [];
  try {
    const data = await cgCall('/search', { query: q });
    return (data.coins || []).slice(0, 10).map(c => ({
      id: c.id, symbol: c.symbol, name: c.name, thumb: c.thumb || c.large || ''
    }));
  } catch (e) { console.warn('search failed', e); return []; }
}

// Поточні ціни для набору монет: cgIds → { id: price_usd }
async function cgPrices(cgIds) {
  if (!cgIds || cgIds.length === 0) return {};
  try {
    const data = await cgCall('/simple/price', {
      ids: cgIds.join(','),
      vs_currencies: 'usd',
    ...
    for (const id in data) {
      out[id] = { price: data[id].usd || 0, change24h: data[id].usd_24h_change || 0 };
    }
    return out;
  } catch (e) { console.warn('prices failed', e); return {}; }
}

// Історія ціни: cgId + days → [[timestamp, price], ...]
async function cgHistory(cgId, days) {
  try {
    const data = await cgCall(`/coins/${cgId}/market_chart`, {
      vs_currency: 'usd',
      days: String(days)
    });
    return data.prices || [];
  } catch (e) { console.warn('history failed', e); return []; }
}
...
function loadState() {
  ...
  if (!s.assetTransactions) s.assetTransactions = [];
  if (!s.assetPrices) s.assetPrices = {};  // cgId → { price, change24h, ts }
  if (s.settings.assetsEnabled == null) s.settings.assetsEnabled = false;
  if (s.settings.assetsCrypto == null) s.settings.assetsCrypto = true;
  if (s.settings.assetsStocks == null) s.settings.assetsStocks = false;
...
function renderWork() {
  ...
  // вигляд list / calendar
  const view = w.viewStyle || 'list';
  document.querySelectorAll('#work-view-switch button').forEach(b =>
    b.classList.toggle('active', b.dataset.wv === view));
  const listEl = document.getElementById('work-list');
  const calEl = document.getElementById('work-calendar');
...
function renderAll() {
  ...
  setTabVis('tab-btn-assets', !!s.assetsEnabled);
  ...
  if (s.assetsEnabled) visibleViews.push('assets');
  ...
  const assetsEl = document.getElementById('view-assets');
  if (assetsEl) assetsEl.style.display = state.view === 'assets' ? '' : 'none';
}

// Заглушка renderAssets — повний UI в наступному пакеті
function renderAssets() {
  if (!document.getElementById('assets-total')) return;
  const totalUSD = state.assets.reduce((s, a) => s + (a.valueUSD || 0), 0);
  const investedUSD = state.assets.reduce((s, a) => s + (a.investedUSD || 0), 0);
  const pnlUSD = totalUSD - investedUSD;
  const pnlPct = investedUSD > 0 ? (pnlUSD / investedUSD) * 100 : 0;
  document.getElementById('assets-total').textContent = formatNum(convert(totalUSD, 'USD', state.display), state.display);
  const pnlEl = document.getElementById('assets-pnl');
  const sign = pnlUSD >= 0 ? '+' : '−';
  pnlEl.textContent = sign + formatNum(convert(Math.abs(pnlUSD), 'USD', state.display), state.display) + ' (' + (pnlUSD >= 0 ? '+' : '') + pnlPct.toFixed(2) + '%)';
  pnlEl.style.color = pnlUSD >= 0 ? 'var(--ok)' : 'var(--danger)';

  document.getElementById('assets-count').textContent = state.assets.length;
  const list = document.getElementById('assets-list');
  if (state.assets.length === 0) {
    list.innerHTML = '<div style="color:var(--muted); text-align:center; padding:20px; font-size:13px;">Ще немає монет. Тапни «+ Додати монету»</div>';
  } else {
    list.innerHTML = state.assets.map(a => {
      const value = formatNum(convert(a.valueUSD, 'USD', state.display), state.display);
      const pnlSign = a.pnlUSD >= 0 ? '+' : '−';
      const pnlCls = a.pnlUSD >= 0 ? 'asset-pnl-positive' : 'asset-pnl-negative';
      const iconHtml = a.icon
        ? `<img src="${escapeHtml(a.icon)}" alt="">`
        : `<span class="asset-icon-fallback">${escapeHtml((a.symbol || '?').slice(0, 3).toUpperCase())}</span>`;
      return `<div class="asset-card" data-asset-id="${a.id}">
        <div class="asset-icon">${iconHtml}</div>
        <div class="asset-body">
          <div class="asset-row1">
            <div><span class="asset-name">${escapeHtml(a.name || a.symbol)}</span><span class="asset-symbol">${escapeHtml(a.symbol)}</span></div>
            <div class="asset-value">${value}</div>
          </div>
          <div class="asset-row2">
            <span>${a.qty.toFixed(6).replace(/\.?0+$/, '')} · $${(a.currentPriceUSD || 0).toFixed(2)}</span>
            <span class="${pnlCls}">${pnlSign}${formatNum(convert(Math.abs(a.pnlUSD), 'USD', state.display), state.display)} (${a.pnlUSD >= 0 ? '+' : ''}${a.pnlPct.toFixed(2)}%)</span>
          </div>
        </div>
      </div>`;
    }).join('');
    list.querySelectorAll('[data-asset-id]').forEach(el => {
      el.addEventListener('click', () => {
        haptic('light');
        openAssetDetail(el.dataset.assetId);
      });
    });
  }
  // Малюємо графік портфеля коли активна вкладка Активи
  if (state.view === 'assets' && state.settings?.assetsEnabled && typeof drawPortfolioChart === 'function') {
    drawPortfolioChart();
  }
}

// Обробник кнопки додати монету
...
function openAssetSearch() {
  const body = document.getElementById('asset-search-body');
  body.innerHTML = `
    <input type="text" class="search-input" id="asset-search-input" placeholder="Bitcoin, ETH, Solana..." autocomplete="off" spellcheck="false">
    <div class="search-results" id="asset-search-results">
      <div style="color:var(--muted); text-align:center; padding:20px; font-size:12px;">Введи назву або символ монети</div>
    </div>
  `;
  document.getElementById('ov-asset-search').classList.add('open');
  const inp = document.getElementById('asset-search-input');
  const resDiv = document.getElementById('asset-search-results');
  ...
  inp.addEventListener('input', () => {
    ...
    if (q.length < 2) {
      resDiv.innerHTML = '<div style="color:var(--muted); text-align:center; padding:20px; font-size:12px;">Введи ще символи...</div>';
      return;
    }
    resDiv.innerHTML = '<div style="color:var(--muted); text-align:center; padding:20px; font-size:12px;">Шукаю...</div>';
    assetSearchDebounce = setTimeout(async () => {
      const coins = await cgSearch(q);
      if (coins.length === 0) {
        resDiv.innerHTML = '<div style="color:var(--muted); text-align:center; padding:20px; font-size:12px;">Нічого не знайдено</div>';
        return;
      }
      resDiv.innerHTML = coins.map(c => `
        <div class="search-result" data-cg-id="${c.id}" data-symbol="${escapeHtml(c.symbol)}" data-name="${escapeHtml(c.name)}" data-icon="${escapeHtml(c.thumb)}">
          <img src="${escapeHtml(c.thumb)}" class="sr-icon" alt="" onerror="this.style.display='none'">
          <div class="sr-name">${escapeHtml(c.name)}</div>
          <div class="sr-symbol">${escapeHtml(c.symbol)}</div>
        </div>
      `).join('');
      resDiv.querySelectorAll('[data-cg-id]').forEach(el => {
        el.addEventListener('click', () => {
          haptic('light');
          const meta = {
            cgId: el.dataset.cgId,
            symbol: (el.dataset.symbol || '').toUpperCase(),
            name: el.dataset.name,
            icon: el.dataset.icon
          };
          document.getElementById('ov-asset-search').classList.remove('open');
          setTimeout(() => openAssetTxModal(null, 'buy', meta), 100);
        });
      });
    }, 350);
  });
...
// ==================== TX МОДАЛКА (купівля/продаж) ====================
function openAssetTxModal(assetId, type, coinMeta) {
  // assetId: null → нова монета (з coinMeta)
  // assetId: існує → покупка/продаж додаткова
  editingAssetId = assetId;
  const isNewAsset = !assetId;
  const asset = isNewAsset ? null : state.assets.find(a => a.id === assetId);
  const meta = coinMeta || (asset ? { cgId: asset.cgId, symbol: asset.symbol, name: asset.name, icon: asset.icon } : null);
  if (!meta) { toast('Помилка: немає інформації про монету', 'error'); return; }

  const currentPrice = state.assetPrices[meta.cgId]?.price;

  document.getElementById('asset-tx-title').textContent =
    isNewAsset ? `Купівля ${meta.symbol}` :
    (type === 'buy' ? `+ Купити ${meta.symbol}` : `− Продати ${meta.symbol}`);

  const body = document.getElementById('asset-tx-body');
  body.innerHTML = `
    <div class="drill-summary" style="display:flex; align-items:center; gap:10px;">
      ${meta.icon ? `<img src="${escapeHtml(meta.icon)}" style="width:32px; height:32px; border-radius:50%;">` : ''}
      <div>
        <div style="font-weight:700;">${escapeHtml(meta.name)}</div>
        <div style="font-family:'Geist Mono',monospace; font-size:11px; color:var(--muted);">${escapeHtml(meta.symbol)} ${currentPrice ? '· зараз $' + currentPrice.toFixed(2) : ''}</div>
      </div>
    </div>

    <div class="field-group">
      <label>Кількість</label>
      <input class="fld" id="at-qty" type="number" min="0" step="any" inputmode="decimal" placeholder="0.05">
    </div>
    <div class="field-group">
      <label>Ціна за 1 ${meta.symbol}</label>
      <div class="two-col">
        <div><input class="fld" id="at-price" type="number" min="0" step="any" inputmode="decimal" placeholder="${currentPrice ? currentPrice.toFixed(2) : '30000'}" value="${currentPrice ? currentPrice.toFixed(2) : ''}"></div>
        <div>
          <select class="fld" id="at-cur">
    ...
    <div class="field-group">
      <label>Дата</label>
      <input class="fld" id="at-date" type="date" value="${toISO(today())}">
    </div>
    <div class="field-group">
      <label>Комісія (опційно)</label>
      <input class="fld" id="at-fee" type="number" min="0" step="any" inputmode="decimal" placeholder="0">
    </div>
    <div class="field-group">
      <label>Опис (опційно)</label>
      <input class="fld" id="at-note" type="text" placeholder="напр. з Binance">
    </div>

    <div id="at-preview" style="padding:10px; background:var(--surface-2); border-radius:10px; margin-bottom:10px; font-family:'Geist Mono',monospace; font-size:12px; color:var(--muted); text-align:center;">
      Разом: —
    </div>

    <button class="btn-primary" id="at-save" style="width:100%; padding:16px; font-size:15px;">${type === 'buy' ? '+ Додати покупку' : '− Додати продаж'}</button>
  `;
  document.getElementById('ov-asset-tx').classList.add('open');
  ...
  const updatePreview = () => {
    ...
    const cur = document.getElementById('at-cur').value;
    const total = qty * price + fee;
    document.getElementById('at-preview').textContent =
      qty > 0 && price > 0 ? `Разом: ${formatNum(total, cur)}` : 'Разом: —';
  };
  ['at-qty', 'at-price', 'at-fee', 'at-cur'].forEach(id => {
    document.getElementById(id).addEventListener('input', updatePreview);
    document.getElementById(id).addEventListener('change', updatePreview);
  });

  document.getElementById('at-save').addEventListener('click', async () => {
    const qty = parseFloat(document.getElementById('at-qty').value);
    ...
    if (!qty || qty <= 0) { toast('Введи кількість', 'error'); return; }
    if (!price || price <= 0) { toast('Введи ціну', 'error'); return; }
    if (!date) { toast('Вкажи дату', 'error'); return; }

    let workAssetId = assetId;
    // Створюємо новий актив якщо треба
    if (isNewAsset) {
      const newA = {
        ...
        icon: meta.icon,
        transactions: [],
        qty: 0, avgBuyUSD: 0, investedUSD: 0,
        currentPriceUSD: currentPrice || 0,
        valueUSD: 0, pnlUSD: 0, pnlPct: 0
      };
      state.assets.push(newA);
      workAssetId = newA.id;
      saveState();
      await createAssetInNotion(newA);
      workAssetId = newA.id;
      saveState();
    }
    ...
    const tx = {
      ...
      assetId: workAssetId,
      date, txType: type, qty, price, cur, fee, note
    };
    state.assetTransactions.push(tx);
    const a = state.assets.find(x => x.id === workAssetId);
    if (a) {
      a.transactions.unshift(tx);
      recalcAssetStats(a);
    }
    saveState();
    createAssetTxInNotion(workAssetId, tx).then(() => saveState());

    // Оновлюємо ціну одразу
    if (a && !state.assetPrices[a.cgId]) {
      refreshAssetPrices().then(() => renderAll());
    }
    haptic('success');
    toast(type === 'buy' ? 'Покупка додана' : 'Продаж додано', 'success');
    document.getElementById('ov-asset-tx').classList.remove('open');
    renderAll();
  });
}

// ==================== ДЕТАЛЬ МОНЕТИ + ГРАФІК ====================
let openAssetIdCurrent = null;
let assetChartDays = 30;

async function openAssetDetail(assetId) {
  openAssetIdCurrent = assetId;
  assetChartDays = 30;
  const a = state.assets.find(x => x.id === assetId);
  if (!a) return;
  document.getElementById('asset-detail-title').textContent = `${a.name || a.symbol}`;
  await renderAssetDetail();
  document.getElementById('ov-asset-detail').classList.add('open');
}

async function renderAssetDetail() {
  const a = state.assets.find(x => x.id === openAssetIdCurrent);
  if (!a) return;
  const body = document.getElementById('asset-detail-body');

  const currentPrice = a.currentPriceUSD || 0;
  const priceCh = state.assetPrices[a.cgId]?.change24h || 0;
  const priceChCls = priceCh >= 0 ? 'asset-pnl-positive' : 'asset-pnl-negative';
  const pnlCls = a.pnlUSD >= 0 ? 'asset-pnl-positive' : 'asset-pnl-negative';

  const txHtml = a.transactions.length === 0
    ? '<div style="color:var(--muted); text-align:center; padding:16px; font-size:12px;">Історії немає</div>'
    : a.transactions.map(t => {
        const sign = t.txType === 'buy' ? '+' : '−';
        const color = t.txType === 'buy' ? 'var(--ok)' : 'var(--danger)';
        const total = t.qty * t.price + (t.fee || 0);
        return `<div class="drill-row" data-tx-id="${t.id}">
          <div class="dr-body">
            <div class="dr-title" style="color:${color};">${sign}${t.qty.toFixed(6).replace(/\.?0+$/, '')} ${a.symbol}</div>
            <div class="dr-meta">${fmtDate(parseDate(t.date))} · ${formatNum(t.price, t.cur)}/шт · разом ${formatNum(total, t.cur)}${t.note ? ' · ' + escapeHtml(t.note) : ''}</div>
          </div>
          <button class="dr-del" data-del-tx="${t.id}" style="background:none; border:none; color:var(--muted); font-size:18px; cursor:pointer;">×</button>
        </div>`;
      }).join('');
  ...
  body.innerHTML = `
    <div class="drill-summary">
      <div style="display:flex; align-items:center; gap:10px;">
        ${a.icon ? `<img src="${escapeHtml(a.icon)}" style="width:32px; height:32px; border-radius:50%;">` : ''}
        <div style="flex:1;">
          <div style="font-weight:700; font-size:15px;">${escapeHtml(a.name || a.symbol)}</div>
          <div style="font-family:'Geist Mono',monospace; font-size:11px; color:var(--muted);">
            $${currentPrice.toFixed(2)} <span class="${priceChCls}">${priceCh >= 0 ? '+' : ''}${priceCh.toFixed(2)}%</span> (24г)
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="chart-wrap" style="margin-top:12px;">
      <div class="period-pick" id="asset-detail-period">
        <button data-d="7" ${assetChartDays === 7 ? 'class="active"' : ''}>7д</button>
        <button data-d="30" ${assetChartDays === 30 ? 'class="active"' : ''}>30д</button>
        <button data-d="90" ${assetChartDays === 90 ? 'class="active"' : ''}>90д</button>
        <button data-d="365" ${assetChartDays === 365 ? 'class="active"' : ''}>1р</button>
      </div>
      <div id="asset-detail-chart" style="min-height:100px;">
        <div style="color:var(--muted); text-align:center; padding:20px; font-size:12px;">Завантажую графік...</div>
      </div>
    </div>

    <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin:12px 0; font-family:'Geist Mono',monospace; font-size:12px;">
      <div style="padding:10px; background:var(--surface-2); border-radius:10px;">
        <div style="color:var(--muted); font-size:10px;">КІЛЬКІСТЬ</div>
        <div style="font-weight:700; font-size:14px; margin-top:2px;">${a.qty.toFixed(6).replace(/\.?0+$/, '')}</div>
      </div>
      <div style="padding:10px; background:var(--surface-2); border-radius:10px;">
        <div style="color:var(--muted); font-size:10px;">СЕРЕДНЯ ЦІНА</div>
        <div style="font-weight:700; font-size:14px; margin-top:2px;">$${a.avgBuyUSD.toFixed(2)}</div>
      </div>
      <div style="padding:10px; background:var(--surface-2); border-radius:10px;">
        <div style="color:var(--muted); font-size:10px;">ВКЛАДЕНО</div>
        <div style="font-weight:700; font-size:14px; margin-top:2px;">${formatNum(convert(a.investedUSD, 'USD', state.display), state.display)}</div>
      </div>
      <div style="padding:10px; background:var(--surface-2); border-radius:10px;">
        <div style="color:var(--muted); font-size:10px;">ВАРТІСТЬ</div>
        <div style="font-weight:700; font-size:14px; margin-top:2px;">${formatNum(convert(a.valueUSD, 'USD', state.display), state.display)}</div>
      </div>
    </div>

    <div style="padding:12px; border-radius:12px; background:${a.pnlUSD >= 0 ? 'rgba(95,217,127,0.08)' : 'rgba(255,77,77,0.08)'}; margin-bottom:12px;">
      <div style="font-family:'Geist Mono',monospace; font-size:10px; color:var(--muted); letter-spacing:1.5px;">P&L</div>
      <div class="${pnlCls}" style="font-family:'Geist Mono',monospace; font-weight:800; font-size:20px; margin-top:2px;">
        ${a.pnlUSD >= 0 ? '+' : '−'}${formatNum(convert(Math.abs(a.pnlUSD), 'USD', state.display), state.display)}
        <span style="font-size:13px;">(${a.pnlUSD >= 0 ? '+' : ''}${a.pnlPct.toFixed(2)}%)</span>
      </div>
    </div>

    <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:10px;">
      <button class="btn-primary" id="a-buy-more" style="background:var(--ok); color:#000;">+ Купити ще</button>
      ${a.qty > 0 ? '<button class="btn-primary" id="a-sell" style="background:transparent; color:var(--danger); border:1px solid var(--danger);">− Продати</button>' : '<div></div>'}
    </div>
    <button class="bd-edit-btn" id="a-delete" style="width:100%; color:var(--danger); border-color:var(--danger);">Видалити монету</button>

    <div style="font-family:'Geist Mono',monospace; font-size:10px; color:var(--muted); letter-spacing:1.5px; margin:16px 0 8px;">ІСТОРІЯ · ${a.transactions.length}</div>
    ${txHtml}
  `;

  // обробники
  document.getElementById('a-buy-more').addEventListener('click', () => {
    haptic('light');
    setTimeout(() => openAssetTxModal(a.id, 'buy'), 100);
  });
  const sellBtn = document.getElementById('a-sell');
  if (sellBtn) sellBtn.addEventListener('click', () => {
    haptic('light');
    setTimeout(() => openAssetTxModal(a.id, 'sell'), 100);
  });
  document.getElementById('a-delete').addEventListener('click', async () => {
    const ok = await confirmDialog({
      icon: '🗑', title: 'Видалити монету?',
      text: 'Разом з усією історією транзакцій', okText: 'Видалити'
    });
    if (!ok) return;
    const aid = a.id;
    state.assets = state.assets.filter(x => x.id !== aid);
    state.assetTransactions = state.assetTransactions.filter(t => t.assetId !== aid);
    saveState();
    // архівуємо транзакції в Notion
    for (const t of a.transactions) deleteAssetTxInNotion(t.id);
    deleteAssetInNotion(aid);
    haptic('warning');
    toast('Монету видалено', 'warn');
    document.getElementById('ov-asset-detail').classList.remove('open');
    renderAll();
  });

  document.getElementById('asset-detail-period').addEventListener('click', e => {
    const b = e.target.closest('button');
    if (!b) return;
    haptic('light');
    assetChartDays = parseInt(b.dataset.d);
    renderAssetDetail();
  });

  // видалення транзакції
  body.querySelectorAll('[data-del-tx]').forEach(btn => {
    btn.addEventListener('click', async (ev) => {
      ev.stopPropagation();
      const ok = await confirmDialog({
        icon: '×', title: 'Видалити транзакцію?', okText: 'Видалити'
      });
      if (!ok) return;
      const txId = btn.dataset.delTx;
      a.transactions = a.transactions.filter(t => t.id !== txId);
      state.assetTransactions = state.assetTransactions.filter(t => t.id !== txId);
      recalcAssetStats(a);
      saveState();
      deleteAssetTxInNotion(txId);
      haptic('light');
      toast('Транзакція видалена', 'warn');
      renderAssetDetail();
      renderAll();
    });
  });
  ...
  // Малюємо графік ціни
  drawAssetPriceChart(a.cgId, assetChartDays);
}

// ==================== ГРАФІКИ (SVG) ====================
async function drawAssetPriceChart(cgId, days) {
  const wrap = document.getElementById('asset-detail-chart');
  if (!wrap) return;
  const data = await cgHistory(cgId, days);
  if (!data || data.length < 2) {
    wrap.innerHTML = '<div style="color:var(--muted); text-align:center; padding:20px; font-size:12px;">Немає даних</div>';
    return;
  }
  ...
  const cls = prices[prices.length - 1] >= prices[0] ? 'positive' : 'negative';
  wrap.innerHTML = `
    <svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" style="height:100px;">
      <polygon class="chart-fill ${cls}" points="${polyPts}"></polygon>
      <polyline class="chart-line ${cls}" points="${pts}"></polyline>
    </svg>
    <div style="display:flex; justify-content:space-between; font-family:'Geist Mono',monospace; font-size:10px; color:var(--muted); margin-top:4px;">
...
// Графік балансу портфеля (Активи вкладка)
let portfolioChartDays = 30;

async function drawPortfolioChart() {
  const wrap = document.getElementById('assets-chart');
  const pnlWrap = document.getElementById('assets-pnl-chart');
  if (!wrap) return;
  if (state.assets.length === 0) {
    wrap.innerHTML = '';
    if (pnlWrap) pnlWrap.innerHTML = '';
    return;
  }
  wrap.innerHTML = '<div style="color:var(--muted); text-align:center; padding:20px; font-size:12px;">Завантажую...</div>';

  // Забираємо історичні ціни для кожної монети
  const histories = {};
  for (const a of state.assets) {
    if (!a.cgId) continue;
    try {
      histories[a.cgId] = await cgHistory(a.cgId, portfolioChartDays);
    } catch (e) { histories[a.cgId] = []; }
  }

  // Будуємо точки: для кожного дня рахуємо баланс за поточними даними
  // Спрощення: беремо перший часовий ряд як таймлайн, для інших монет — інтерполюємо найближчі
  const timeline = histories[state.assets[0].cgId] || [];
  ...
  const balancePts = [];
  const pnlPts = [];
  for (const [ts, _] of timeline) {
    let value = 0;
    let invested = 0;
    for (const a of state.assets) {
      // кількість на дату ts
      let qty = 0, cost = 0;
      for (const t of a.transactions) {
        const tt = new Date(t.date).getTime();
        if (tt > ts) continue;
        const priceUSD = convert(t.price, t.cur || 'USD', 'USD');
        if (t.txType === 'buy') { qty += t.qty; cost += t.qty * priceUSD; }
        else { qty -= t.qty; cost -= (qty > 0 ? cost / (qty + t.qty) : 0) * t.qty; }
      }
      if (qty <= 0) continue;
      // ціна на дату ts з history
      const hist = histories[a.cgId] || [];
      let priceAtTs = a.currentPriceUSD || 0;
      for (const [ht, p] of hist) {
        if (ht <= ts) priceAtTs = p;
        else break;
      }
      value += qty * priceAtTs;
      invested += cost;
    }
    balancePts.push([ts, value]);
    pnlPts.push([ts, value - invested]);
  }

  drawSvgChart(wrap, balancePts, 'Баланс', 120, false);
  if (pnlWrap) drawSvgChart(pnlWrap, pnlPts, 'P&L', 60, true);
}

function drawSvgChart(wrap, pts, label, height, colorByZero) {
  if (!pts || pts.length < 2) { wrap.innerHTML = '<div style="color:var(--muted); text-align:center; padding:14px; font-size:11px;">—</div>'; return; }
  ...
  const polyPts = `0,${height} ${svgPts} ${w},${height}`;
  let cls = 'positive';
  if (colorByZero) cls = vals[vals.length - 1] >= 0 ? 'positive' : 'negative';
  else cls = vals[vals.length - 1] >= vals[0] ? 'positive' : 'negative';
  const last = vals[vals.length - 1];
  wrap.innerHTML = `
    <div style="font-family:'Geist Mono',monospace; font-size:10px; color:var(--muted); letter-spacing:1.5px; margin-bottom:4px;">${label.toUpperCase()} · ${formatNum(convert(last, 'USD', state.display), state.display)}</div>
    <svg viewBox="0 0 ${w} ${height}" preserveAspectRatio="none" style="height:${height}px; width:100%;">
      <polygon class="chart-fill ${cls}" points="${polyPts}"></polygon>
      <polyline class="chart-line ${cls}" points="${svgPts}"></polyline>
    </svg>
  `;
}

// Обробник періоду графіку портфеля
document.getElementById('assets-chart-period')?.addEventListener('click', e => {
  const b = e.target.closest('button');
  if (!b) return;
  haptic('light');
  document.querySelectorAll('#assets-chart-period button').forEach(x => x.classList.toggle('active', x === b));
  portfolioChartDays = parseInt(b.dataset.d);
  drawPortfolioChart();
});
...
function ensureCalState() {
  if (!state.work.viewStyle) state.work.viewStyle = 'list';
  if (!state.work.calendarAnchor) state.work.calendarAnchor = toISOsafe(new Date());
}

function renderWorkCalendar() {
  ensureCalState();
  const anchor = new Date(state.work.calendarAnchor);
  const year = anchor.getFullYear();
  const month = anchor.getMonth();

  const titleEl = document.getElementById('cal-title');
  if (titleEl) titleEl.textContent = UA_MONTHS[month] + ' ' + year;

  // збираємо агрегати по днях місяця
  const daysMap = {};  // 'YYYY-MM-DD' → { hours, usd, entries }
  for (const e of state.work.entries) {
    const d = new Date(e.date);
    if (d.getFullYear() !== year || d.getMonth() !== month) continue;
    const key = e.date.slice(0, 10);
    if (!daysMap[key]) daysMap[key] = { hours: 0, usd: 0, entries: [] };
    daysMap[key].entries.push(e);
    if (e.kind === 'hourly') daysMap[key].hours += (e.hours || 0);
    daysMap[key].usd += workEntryUSD(e);
  }

  // сітка: додаємо порожні клітинки на початку до понеділка
  const firstDay = new Date(year, month, 1);
  const firstWd = (firstDay.getDay() + 6) % 7; // ПН=0
  ...
  for (let d = 1; d <= lastDay; d++) {
    ...
    const data = daysMap[dateKey];
    cells.push({ day: d, date: dateKey, data });
  }
  ...
  grid.innerHTML = cells.map(c => {
    ...
    if (!c.data) {
      return `<div class="cal-cell empty" data-date="${c.date}">
        <div class="cal-day-num">${c.day}</div>
      </div>`;
    }
    const usdDisp = convert(c.data.usd, 'USD', state.display);
    // компактне число: 90 замість 90.00, 1.2k якщо >999
    const usdShort = usdDisp >= 1000 ? (usdDisp/1000).toFixed(1)+'k' : Math.round(usdDisp);
    const curSign = { USD:'$', UAH:'₴', PLN:'zł' }[state.display] || '';
    return `<div class="cal-cell has-work" data-date="${c.date}">
      <div class="cal-day-num">${c.day}</div>
      <div class="cal-day-data">
        ${c.data.hours > 0 ? `<span class="cd-h">${c.data.hours.toFixed(1).replace(/\.0$/,'')}г</span>` : ''}
        <span class="cd-v">${state.display === 'USD' || state.display === 'PLN' ? curSign+usdShort : usdShort+curSign}</span>
      </div>
    </div>`;
  }).join('');

  // тапи по клітинках з роботою
  grid.querySelectorAll('.cal-cell.has-work[data-date]').forEach(el => {
    el.addEventListener('click', () => {
      haptic('light');
      openCalendarDayDetail(el.dataset.date);
    });
  });
}

// Драйв-ін дня — переваикористовуємо existing drill модалку
function openCalendarDayDetail(dateStr) {
  const dateD = new Date(dateStr);
  const entries = state.work.entries.filter(e => e.date.slice(0,10) === dateStr);
  if (entries.length === 0) return;

  const catName = (catId) => {
    if (!catId) return 'без категорії';
    const c = state.workCategories.find(x => x.id === catId);
    return c ? c.name : 'категорія';
  };

  const totalUSD = entries.reduce((s, e) => s + workEntryUSD(e), 0);
  const totalHours = entries.reduce((s, e) => s + (e.kind === 'hourly' ? (e.hours||0) : 0), 0);

  document.getElementById('drill-title').textContent = fmtDate(dateD);
  const body = document.getElementById('drill-body');
  body.innerHTML = `
    <div class="drill-summary">
      <div style="font-family:'Geist Mono',monospace; font-weight:800; font-size:22px; color:var(--yellow);">
        ${formatNum(convert(totalUSD, 'USD', state.display), state.display)}
      </div>
      <div style="font-family:'Geist Mono',monospace; font-size:11px; color:var(--muted); margin-top:2px;">
        ${totalHours > 0 ? totalHours.toFixed(1).replace(/\.0$/,'') + ' год · ' : ''}${entries.length} ${entries.length === 1 ? 'запис' : 'записів'}
      </div>
    </div>
    <div style="font-family:'Geist Mono',monospace; font-size:10px; color:var(--muted); letter-spacing:1.5px; margin:14px 0 6px;">ЗАПИСИ</div>
    ${entries.map(e => {
      const usd = workEntryUSD(e);
      const title = e.kind === 'hourly'
        ? `${(e.hours||0).toFixed(1).replace(/\.0$/,'')} год · ${catName(e.categoryId)}`
        : (e.title || 'Підробіток');
      return `<div class="drill-row" data-edit-entry="${e.id}">
        <div class="dr-body">
          <div class="dr-title">${escapeHtml(title)}</div>
          <div class="dr-meta">${formatNum(convert(usd, 'USD', state.display), state.display)}${e.note ? ' · ' + escapeHtml(e.note) : ''}</div>
        </div>
      </div>`;
    }).join('')}
  `;
  ...
  // клік на запис → редагування
  body.querySelectorAll('[data-edit-entry]').forEach(el => {
    el.addEventListener('click', () => {
      haptic('light');
      document.getElementById('ov-drill').classList.remove('open');
      setTimeout(() => openEditWorkEntry(el.dataset.editEntry), 100);
    });
  });
}

// Обробники перемикача і навігації
document.getElementById('work-view-switch')?.addEventListener('click', e => {
  const b = e.target.closest('button');
  if (!b) return;
  ensureCalState();
  haptic('light');
  state.work.viewStyle = b.dataset.wv;
  saveState();
  renderWork();
});
document.getElementById('cal-prev')?.addEventListener('click', () => {
  ensureCalState();
  const d = new Date(state.work.calendarAnchor);
  d.setMonth(d.getMonth() - 1);
  state.work.calendarAnchor = toISOsafe(d);
  haptic('light');
  saveState();
  renderWorkCalendar();
});
...
// Тумблер Активи
const assetsToggle = document.getElementById('assets-enabled-toggle');
if (assetsToggle) {
  assetsToggle.checked = !!state.settings?.assetsEnabled;
  const updateAssetsSub = () => {
    const el = document.getElementById('mi-assets-sub');
    if (el) el.textContent = state.settings?.assetsEnabled ? 'Увімкнено' : 'Вимкнено';
  };
  updateAssetsSub();
  assetsToggle.addEventListener('change', async () => {
    if (!state.settings) state.settings = {};
    state.settings.assetsEnabled = assetsToggle.checked;
    saveState();
    updateAssetsSub();
    haptic('light');
    renderAll();
    if (assetsToggle.checked) {
      toast('Завантажую активи...', 'info');
      await loadAssetsFromNotion();
      renderAll();
    }
  });
}
