...
function renderAssets() {
  ...
  if (state.assets.length === 0) {
    ...
    // Обробники — детальний перегляд в наступному пакеті
    list.querySelectorAll('[data-asset-id]').forEach(el => {
      el.addEventListener('click', () => {
        haptic('light');
        // openAssetDetail(el.dataset.assetId); — наступним пакетом
        toast('Деталь монети — в наступному оновленні', 'info');
      });
    });
  }
}
...
document.getElementById('btn-add-asset')?.addEventListener('click', () => {
  haptic('light');
  toast('Пошук монет — в наступному оновленні', 'info');
});
